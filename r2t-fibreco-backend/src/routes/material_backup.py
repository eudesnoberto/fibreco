...
# Sistema de progresso em memória
pdf_progress = {}
...
def get_progress(task_id):
    ...
    with progress_lock:
        return pdf_progress.get(task_id, {'progress': 0, 'message': 'Iniciando...', 'timestamp': time.time()})

def clear_progress(task_id):
    """Limpar progresso de uma tarefa"""
    with progress_lock:
        if task_id in pdf_progress:
            del pdf_progress[task_id]

def gerar_pdf_atividade_com_progresso(atividade_id, task_id):
    """Gerar PDF da atividade com atualizações de progresso"""
    ...
    try:
        ...
        update_progress(task_id, 10, "Preparando documento...")
        
        # Criar buffer para o PDF
        buffer = io.BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=A4, rightMargin=40, leftMargin=40, topMargin=40, bottomMargin=40)
        
        update_progress(task_id, 20, "Configurando estilos...")
        ...
        update_progress(task_id, 30, "Adicionando cabeçalho...")
        ...
        story.append(Paragraph("Relatório de Atividade", subtitle_style))
        story.append(Spacer(1, 20))
        
        update_progress(task_id, 40, "Adicionando informações da atividade...")
        
        # Informações da atividade
        info_data = [
            ['Título:', atividade.titulo],
            ['Data de Criação:', atividade.data_criacao.strftime('%d/%m/%Y %H:%M')],
            ['Status:', atividade.status.title()],
            ['Responsável:', atividade.usuario.nome if atividade.usuario else 'N/A']
        ]
        ...
        update_progress(task_id, 50, "Processando materiais utilizados...")
        ...
        update_progress(task_id, 60, "Processando imagens...")
        ...
        if atividade.imagens_conclusao:
            ...
            if imagens:
                ...
                update_progress(task_id, 70, f"Processando {len(imagens_validas)} imagens...")
                ...
                for i in range(0, len(imagens_validas), 2):
                    ...
                    # Atualizar progresso baseado no número de imagens processadas
                    progress_imagens = 70 + int((imagens_processadas / len(imagens_validas)) * 20)
                    update_progress(task_id, progress_imagens, f"Processando imagens... {imagens_processadas}/{len(imagens_validas)}")
        ...
        update_progress(task_id, 90, "Finalizando documento...")
        
        # Construir PDF
        doc.build(story)
        
        update_progress(task_id, 100, "PDF gerado com sucesso!")
        ...
        update_progress(task_id, -1, f"Erro na geração: {str(e)}")
        raise e
...
def update_material(material_id):
    ...
    if nova_quantidade is not None and nova_quantidade != quantidade_anterior:
...
def gerar_pdf_atividade(atividade_id):
    ...
    def gerar_pdf_thread():
        try:
            return gerar_pdf_atividade_com_progresso(atividade_id, task_id)
        except Exception as e:
            update_progress(task_id, -1, f"Erro: {str(e)}")
            return None
    
    # Iniciar thread
    thread = threading.Thread(target=gerar_pdf_thread)
    thread.start()
    
    # Retornar task_id para o frontend acompanhar o progresso
    return jsonify({
        'task_id': task_id,
...
@material_bp.route('/pdf/progress/<task_id>', methods=['GET'])
@login_required
def get_pdf_progress(task_id):
    """Obter progresso da geração de PDF"""
